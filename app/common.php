...
if (!function_exists('sort_list_tier')) {
     ...
     * @return array
     */
    function sort_list_tier($data, $pid = 0, $field = 'pid', $pk = 'id', $html = '|-----', $level = 1, &$list = [])
    {
        foreach ($data as $k => $res) {
            if ($res[$field] == $pid) {
                ...
                $list[]      = $res;
                unset($data[$k]);
                sort_list_tier($data, $res[$pk], $field, $pk, $html, $level + 1, $list);
            }
        }
        return $list;
    }
...
if (!function_exists('link_url')) {
     ...
     * @return string
     */
    function link_url(string $link)
    {
        if (!$link) {
            return '';
        }
        if (strstr($link, 'http') === false) {
            return sys_config('site_url') . $link;
        } else {
            return $link;
        }
    }
...
if (!function_exists('swoole_go')) {

    /**
     *
     * @param callable $fn
     */
    function swoole_go(callable $fn)
    {
        if (is_win()) {
            $fn();
        } else {
