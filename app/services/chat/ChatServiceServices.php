...
class ChatServiceServices extends BaseServices
{
    ...
    public function getRecord(string $appId, array $user, int $idTo, int $limit = 10, int $toUserId = 0, int $cookieUid = 0, int $kefuId = 0, int $kefuRand = 0)
    {
        ...
        if (!$toUserId) {
            ...
            if (!count($serviceInfoList)) {
                throw new ValidateException(lang('There is currently no customer service staff online, please contact us later'));
            }
    ...
    public function getCustomerServiceList(string $appId){
        $where=['appid'=>$appId];
       return  $this->dao->getCustomerServiceList($where,"user_id,nickname");
    }
