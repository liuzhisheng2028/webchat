...
class ApplicationServices extends BaseServices
{
    ...
    public function createUser(string $appid, array $userData = [])
    {
        ...
        if ($uid && ($userInfo = $userServices->get(['uid' => $uid, 'appid' => $appid]))) {
            ...
            $userInfo->last_ip_name = \get_ip_name($userInfo->last_ip);
            $userInfo->save();
            ...
            $userInfo = $userServices->save([
                ...
                'last_ip_name' => \get_ip_name(app('request')->ip())
            ]);
            if (!$userInfo) {
    ...
    /**
     * @param string $token_md5
     * @return int
     */
    public function getApplicationId(string $token_md5)
    {
        return $this->dao->getOne(['token_md5'=>$token_md5,'is_delete'=>0]);
    }
}
